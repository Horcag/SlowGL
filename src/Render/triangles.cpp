...
void sgl::render::drawTriangleTransform(sgl::ColorTexture&target, const glm::mat4 transform, const glm::vec3&v0,
                                        const glm::vec3&v1, const glm::vec3&v2, sf::Color color) {
...
void sgl::render::drawTrianglesBuffer(sgl::ColorTexture&target, const glm::mat4 transform,
                                      const std::vector<glm::vec3> vertex, const std::vector<glm::uvec3> indices) {
...
void sgl::render::drawTrianglesZBuffer(sgl::ColorTexture&target, sgl::DepthTexture&depth, const glm::mat4 transform,
                                       const std::vector<glm::vec3> vertex, const std::vector<glm::uvec3> indices) {
    depth.clear(-FLT_MAX);
    glm::vec2 image_size = target.getSize();
    std::vector<glm::vec3> transformed(vertex.size(), glm::vec3());
    for (size_t i = 0; i < transformed.size(); i++) {
        transformed[i] = (glm::vec3(transform * glm::vec4(vertex[i], 1.f)) + glm::vec3(1.f, 1.f, 0.f)) *
                         glm::vec3(image_size, 2.f) / 2.f;
        //target.setPixel(*transformed, sf::Color::Blue);
    }
